...
import { hexToBytes, bytesToHex, normalizeAddress, normalizeSlotKey } from "./hex.js";
import { verifyAccountProof, verifyStorageProof } from "./mpt.js";
import { fetchProof, fetchCode } from "./rpc-client.js";
...
export class VerifiedStateBackend {
    ...
    async getAccountBasic(address) {
        ...
        if (cached)
            return cached;
        const proof = await fetchProof(this.proofServerUrl, addr, [], this.blockNumber);
    ...
    async getCode(address) {
        ...
        const codeHex = await fetchCode(this.codeSourceUrl, normalizeAddress(address), this.blockNumber);
    ...
    async flushPendingSlots(address) {
        ...
        try {
            ...
            const proof = await fetchProof(this.proofServerUrl, address, uncachedSlots, this.blockNumber);
            ...
            for (const sp of proof.storageProof) {
                ...
                const verified = verifyStorageProof(account.storageRoot, slotKeyBytes, sp.proof);
                const cacheKey = `${address}:${slot}`;
    ...
    verifyAndCacheAccount(address, proof) {
        ...
        const result = verifyAccountProof(this.stateRoot, addrBytes, proof.accountProof);
        if (!result.exists) {
            ...
            const emptyAccount = {
                ...
                nonce: 0n,
                balance: 0n,
                storageRoot: hexToBytes("0x56e81f171bcc55a6ff8345e692c0f86e5b48e01b996cadc001622fb5e363b421"),
...
//# sourceMappingURL=verified-state.js.map