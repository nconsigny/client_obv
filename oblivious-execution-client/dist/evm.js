...
import { hexToBytes, hexToBigInt } from "./hex.js";
 ...
 */
export async function executeCall(params, header, state) {
        ...
        while (pc < ctx.code.length) {
            ...
            switch (op) {
                ...
                case 0x35: {
                    ...
                    for (let j = 0; j < 32; j++) {
                        val <<= 8n;
                        if (i + j < ctx.callData.length)
                            val |= BigInt(ctx.callData[i + j]);
                    }
                    push(val);
                    break;
                }
                ...
                case 0x55: {
                    if (ctx.readOnly)
                        throw new Error("SSTORE in static context");
                    // For eth_call, we can implement ephemeral storage if needed
                    useGas(5000n);
                    pop();
                    pop(); // Discard key and value for now
                    break;
                }
                // 0x56: JUMP
                case 0x56: {
