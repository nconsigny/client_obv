 ...
 */
export function verifyProof(rootHash, path, proofNodes) {
    if (proofNodes.length === 0) {
        ...
        if (bytesEqual(rootHash, emptyRoot))
            return null;
    ...
    for (let i = 0; i < proofNodes.length; i++) {
        ...
        if (nodeBytes.length >= 32) {
            if (!bytesEqual(nodeHash, expectedHash)) {
                throw new Error(`MPT: hash mismatch at proof node ${i}: expected ${bytesToHex(expectedHash)}, got ${bytesToHex(nodeHash)}`);
            }
        }
 ...
 */
export function parseAccountRLP(rlpBytes) {
 ...
 */
export function parseStorageValue(rlpBytes) {
 ...
 */
export function verifyAccountProof(stateRoot, address, accountProof) {
 ...
 */
export function verifyStorageProof(storageRoot, slotKey, storageProofNodes) {
